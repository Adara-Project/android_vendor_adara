...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ROOT"/prebuilts/tools-adara/common/smali/baksmali.jar
        export SMALIJAR="$ROOT"/prebuilts/tools-adara/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ROOT"/prebuilts/tools-adara/"${HOST,,}"-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ROOT"/prebuilts/tools-adara/"${HOST,,}"-x86/bin/compact_dex_converter
    fi
