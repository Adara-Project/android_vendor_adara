...
func init() {
	android.RegisterModuleType("adara_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = adaraExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = adaraExpandVariables(ctx, dep_file)
	...
	cmd := adaraExpandVariables(ctx, String(g.properties.Cmd))
